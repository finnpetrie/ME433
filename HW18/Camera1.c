...
// ... (previous includes and defines remain the same)
...
{ 
    ...
    // Initialize hardware
    init_camera_pins();
    pwm_init_gpio(PWM_L_PIN, PWM_WRAP);   // Left motor PWM
    pwm_init_gpio(PWM_R_PIN, PWM_WRAP);   // Right motor PWM
    
    // Configure direction pins (forward)
    gpio_init(DIR_L_LOW);
    gpio_set_dir(DIR_L_LOW, GPIO_OUT);
    gpio_put(DIR_L_LOW, 0);  // IN2 = LOW (forward)
    
    gpio_init(DIR_R_LOW);
    gpio_set_dir(DIR_R_LOW, GPIO_OUT);
    gpio_put(DIR_R_LOW, 0);  // BIN2 = LOW (forward)
    ...
    while (true)
    {
        ...
        setSaveImage(1);
        while(getSaveImage() == 1) {}  // Wait for capture
        
        convertImage();
        int centre = findLine(IMAGESIZEY/2);  // Find line position
        ...
        int err_px = centre - (IMAGESIZEX/2);
        int dutyL, dutyR;
        compute_pwm(err_px, &dutyL, &dutyR);  // Compute motor duties
        
        // Drive motors with new values
        drive_left(dutyL);
        drive_right(dutyR);
        
        printf("Centre: %d, Error: %d, L: %d, R: %d\n", 
               centre, err_px, dutyL, dutyR);
    }
}
// int main()
// { 
